...
<div class=" w-full min-h-screen relative pb-[30vh]">
	...
	{:else}
		<slot />
		<div
			class=" h-[70vh] hidden  overflow-scroll lg:absolute right-10 top-[50vh] lg:mt-10 lg:w-[35%] bg-white border-2 shadow-none cursor-pointer transition-all space-y-4 shadow-gray-300 rounded-2xl p-10"
		>
			...
			{#each updated as journal}
				...
				>
					<a href={`/journals/category/${journal.short}`} data-sveltekit-preload-data='hover'  class=" hover:text-blue-600 transition-all duration-100">{journal.title}</a>
				</Button>
			{/each}
		</div>
	{/if}
